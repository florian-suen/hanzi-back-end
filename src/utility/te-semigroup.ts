...
interface TEconcat {
    (x:TaskEither<string,CharCollection[]>):(y:TaskEither<string,CharCollection[]>) 
    => TaskEither<string,CharCollection[]>
   }

    const arraySemigroup:Semigroup<Array<CharCollection>> = {concat: (x, y) => x.concat(y)};
    const semiGroupCharColl = getSemigroup<string, CharCollection[]>(arraySemigroup);
